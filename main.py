...
web_app_button = InlineKeyboardMarkup().add(
    InlineKeyboardButton('Bizning sahifamizga utish', web_app=types.WebAppInfo(url='https://al-asal.uz'))
)
...
async def start(message: types.Message):
    ...
    caption_text = ('AsSalomu alaykum! "Al-asal" sahifamizga hush kelibsiz, '
                    'tizimni ishga tushirish uchun kelin tanishib olaylik, uning uchun, '
                    'o`z kontaktingizni junating.')
    await bot.send_photo(chat_id=message.chat.id, photo=photo_url, caption=caption_text, reply_markup=contact)

@dp.message_handler(content_types=types.ContentType.CONTACT)
async def cmd_contact(message: types.Message):
    await message.answer('Bizning sahifamizga utish uchun quyidagi tugmani bosing:', reply_markup=web_app_button)
